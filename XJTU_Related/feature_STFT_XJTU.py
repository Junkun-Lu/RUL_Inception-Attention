...
def vibration_train_picture(train_x_dataframe, train_file_num_ls, train_bearing_data_set): 
    ...
    for i in range(0, len(train_file_num_ls)):
        # 每一个file包含32768个文件
        temp_split_dataframe = train_x_dataframe.iloc[:32768*train_file_num_ls[i], -1]
        train_x_dataframe = train_x_dataframe.iloc[32768*train_file_num_ls[i]:, -1]
    ...
    plt.tight_layout()
    plt.savefig('/content/drive/MyDrive/picture_XJTU/vibration_picture/train_vibration_picture/vibration_of_training_set_{}.jpg'.format(train_bearing_data_set[0].split("_")[0]))  
...
def vibration_test_picture(test_x_dataframe, test_bearing_data_set):
    ...
    plt.ylabel("vibration signal")
    plt.tight_layout()     
    plt.savefig('/content/drive/MyDrive/picture_XJTU/vibration_picture/test_vibration_picture/vibration_of_test_set_{}.jpg'.format(test_bearing_data_set[0])) 


# STFT data calculate:   25.6*10e3
...
def STFT_picture_train(train_stft_feature, train_stft_f, train_stft_t, train_file_num_ls, train_bearing_data_set):
    ...
    for i in range(0, len(train_file_num_ls)):
        ...
        plt.ylabel("Frequency(Hz)")
    plt.tight_layout()
    plt.savefig('/content/drive/MyDrive/picture_XJTU/STFT_picture/train_STFT_picture/STFT_of_training_set_{}.jpg'.format(train_bearing_data_set[0].split("_")[0]))      
...
def STFT_picture_test(test_stft_feature, test_stft_f, test_stft_t, test_bearing_data_set):
    ...
    plt.tight_layout()
    plt.savefig('/content/drive/MyDrive/picture_XJTU/STFT_picture/test_STFT_picture/STFT_test_{}.jpg'.format(test_bearing_data_set[0])) 
